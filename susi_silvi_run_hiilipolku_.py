...
def check_end(n_ditch_scens, new_end_yr, end_yr):
    ...
    for e in range(0, n_ditch_scens):
        if (new_end_yr[e] < end_yr) & (new_end_yr[e]>0):
            end_reached = False
            break
    #for e in range(0, n_ditch_scens): 
...
def call_local_susi_silvi(wdata, mottifile, sarkaSim, ageSim, sfc_mean, sfc_spec, site, break_vol, start_yr_arr, start_yr_ini, end_yr, vol_aft, susi_outfile, ash_year, peat_mass_inputs, peat_N_inputs, peat_P_inputs, peat_K_inputs, nutstat,susiPath, wpath, mottipath, folderName, outfile):
    ...
    new_end_yr = new_susi.run_susi(forc, wpara, cpara, org_para, spara, outpara, photopara, start_yr_arr, end_yr, wlocation = 'undefined', 
                            ...
                            folderName=folderName,ageSim=ageSimD, sarkaSim=sarkaSim, sfc=sfc, susiPath=susiPath, break_vol=break_vol, peat_mass_inputs=peat_mass_inputs, peat_N_inputs=peat_N_inputs, peat_P_inputs=peat_P_inputs, peat_K_inputs=peat_K_inputs, nutstat=nutstat)       
...
def call_local_susi_motti_silvi_list(i,kuviot, puustot, poistumat, tapahtumat, tulot, wdata, sarka, n_ditch_scens, start_yr_ini, totSimYears,susiPath, wpath, mottipath, folderName, outfile):
    ...
    if len(pdata)==0:
        break_vol=None
        j = 0
        new_end_yr, motti_found = call_susi_help(kuviot, stand, j, end_yr, start_yr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, np.nan, sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=ash_year, vol_ini_0=vol_ini_0)
        ...
        while j < n_mottifiles:
            ...
            if j==0:
                ...
                if vuodet[0]<5: # Jos ensimmäinen tapahtuma ennen v.5, vuosille 0-5 ei yleensä pysty muodostaa omaa järkevää motti-tiedostoa
                    ...
                    new_end_yr, motti_found = call_susi_help(kuviot, stand, j, end_yr, start_yr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, vol_after[j], sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=ash_year, vol_ini_0=vol_ini_0,)
            ...
            if (j>0) & (j<n_mottifiles-1):
                ...
                new_end_yr, motti_found = call_susi_help(kuviot, stand, j, end_yr, start_yr_arr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, vol_after[j], sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=ash_year, vol_ini_0=vol_ini_0)    
                end_reached = check_end(n_ditch_scens, new_end_yr, end_yr)
            ...
            if n_mottifiles > 2:
                if j == n_mottifiles-1: # Jos tullaan viimeiseen motti-tiedostoon...
                    ...
                    new_end_yr, motti_found = call_susi_help(kuviot, stand, j, end_yr, start_yr_arr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, np.nan, sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=ash_year, vol_ini_0=vol_ini_0)   #vol_after[j] 
                ...
                if j==1:
                    ...
                    if j==n_mottifiles-1: # Jos viimeinen Motti-tiedosto...    
                        new_end_yr, motti_found = call_susi_help(kuviot, stand, j, end_yr, start_yr_arr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, np.nan, sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=ash_year, vol_ini_0=vol_ini_0)    
...
def call_susi_help(kuviot, stand, j, end_yr, start_yr_arr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, vol_aft, sfc_spec, susiPath, wpath, mottipath, folderName, outfile, ash_year=None, vol_ini_0=None): # maku=None
    ...
    if len(df0)>0:
        ...
        ageSim = df['age'][0]
        if ageSim==0.:
            ageSim=0.001 #oli ennen 0.001, mutta aiheuttiko liikakasvuja?
        ...
        if (treesp==1) | (treesp==2) | (treesp==3) | (treesp==4): # Simulaatio toteutuu vain, jos pÃ¤Ã¤puulaji tiedossa
            ...
            if (subgroup>=2) & (subgroup<=3) & (sfc<=5): # Simulaatio toteutuu vain, jos subgroup 2 tai 3, ja sfc<=5
                ...
                new_end_yr = call_local_susi_silvi(wdata, mottifile, sarkaSim, ageSim, sfc, sfc_spec, site, break_vol, start_yr_arr, start_yr_ini, end_yr, vol_aft, susi_outfile, ash_year, peat_mass_inputs, peat_N_inputs, peat_P_inputs, peat_K_inputs, nutstat,susiPath, wpath, mottipath, folderName, outfile)
