...
def get_paths():
    ...
    outPath=r'/scratch/project_2002470/SUSI_HIILIPOLKU/outputs/' # outputs path
    ...
    mottipath = r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti_files_for_susi/'
    outfile = r'/scratch/project_2002470/SUSI_HIILIPOLKU/outputs/Sorvasranta_BAU_A.txt'    
...
def get_motti_data():
    ...
    kuviot = pd.read_csv(r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti/Sorvasranta_BAU_A_kuviot.csv', encoding='latin1', sep=';')
    puustot = pd.read_csv(r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti/Sorvasranta_BAU_A_puustot.csv', encoding='latin1', sep=';')
    poistumat = pd.read_csv(r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti/Sorvasranta_BAU_A_poistumat.csv', encoding='latin1', sep=';')
    tapahtumat = pd.read_csv(r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti/Sorvasranta_BAU_A_tapahtumat.csv', encoding='latin1', sep=';', index_col=False)
    tulot = pd.read_csv(r'/scratch/project_2002470/SUSI_HIILIPOLKU/Sorvasranta_BAU_A/Sorvasranta_BAU_A_motti/Sorvasranta_BAU_A_kasvut.csv', encoding='latin1', sep=';')
    
    return kuviot, puustot, poistumat, tapahtumat, tulot

def get_inputs():
        
    wdata='sorvasranta_weather.csv' #'puruvesi_saa_19810101-2020062.csv' #'halvansuo_weather.csv' #'kuonanjoki_weather.csv' # Weather data file name
    sarka = 40. # Ditch spacing
    n_ditch_scens = 4 # Number of ditch scenarios 
    start_yr_ini = 1991 # Initial starting year
    totSimYears=30 # Total lenght of simulations, in years
...
def get_sfc_spec(sfc, puulaji):
    ...
    if (sfc==3) & (puulaji=='M�nty'): #jos ajaa suoraan niin voi olla � muta ei sbatch
        sfc_spec = 2
        
    if (sfc==4) & (puulaji=='M�nty'):
        sfc_spec = 1
...
def call_local_susi_silvi(wdata, mottifile, sarkaSim, ageSim, sfc_mean, sfc_spec, site, break_vol, start_yr_arr, start_yr_ini, end_yr, vol_aft, susi_outfile, ash_year):
    ...
    os.rename(folderName + 'susi.nc', folderName + susi_outfile)
...
def call_susi_help(kuviot, stand, j, end_yr, start_yr_arr, wdata, sarka, break_vol, start_yr_ini, n_ditch_scens, vol_aft, sfc_spec, maku=None, ash_year=None, vol_ini_0=None):
    ...
    treesp = pd.read_excel(mottifile['path'] + '/' + mottifile['dominant'][1], sheet_name='Kertym�t')['id Puulaji'][0] # Puulajin pitäisi olla Motti-tiedostossa oikein, vaikka puulaji vaihtuu
