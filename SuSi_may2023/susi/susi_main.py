...
class Susi():
    ...
    def run_susi(self, forc, wpara, cpara, org_para, spara, outpara, photopara, start_yr_arr, end_yr, wlocation=None, mottifile=None, peat=None, 
        ...
        start_date = datetime.datetime(int(min(start_yr_arr)),1,1) 
        end_date=datetime.datetime(end_yr,12,31)
        length = (end_date - start_date).days +1                                   # simulation time in days
        yrs = end_yr - int(min(start_yr_arr)) +1                                                 # simulation time in years
        ...
        print ('      - Weather input:', wpara['description'], ', start:', int(min(start_yr_arr)), ', end:', end_yr) 
        ...
        for r, dr in enumerate(zip(spara['ditch depth west'], spara['ditch depth 20y west'], \
            ...
            # **************  Biogeochemistry ***********************************
                ...
                # If stand volume for cuttings (break_vol) is reached, stop simulation,
                # return the year reached, and proceed to simulation of next ditch depth.
                
                print('Year = ' + str(yr))
                print('Volume=', np.round(stand.volume.mean(),1))
                
                if break_vol:
                    print('Break_vol=',np.round(break_vol,0))
                    if stand.volume.mean() >= break_vol:
                        new_end_yr[r] = yr
                        #tama alla lisatty 23.2.23
                        #
                        #
